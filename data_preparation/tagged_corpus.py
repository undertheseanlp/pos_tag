...
class TaggedCorpus:
    ...
    def _parse_sentence(self, text, skip_column=None):
        ...
        if skip_column is None:
            ...
            tokens = [token.split("\t")[(skip_column + 1):] for token in tokens if token.strip() != ""]
    ...
    def load(self, filepath, skip_column=None):
        ...
        :type skip_column: int 
        :param int skip_column: line numbers to skip (0-indexed)
        """
        with open(filepath) as f:
            content = f.read().strip()
            content = content.decode("utf-8")
            sentences = content.split("\n\n")
            sentences = [self._parse_sentence(s.strip(), skip_column) for s in sentences if s.strip() != ""]
            self.sentences = sentences
